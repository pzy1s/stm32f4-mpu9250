...
{
	if(HALIIC_WriteMultByteToSlave(addr,reg,len,buf))
		return 0;
	else
		return 1;
} 
...
{ 
	if(HALIIC_ReadMultByteFromSlave(addr,reg,len,buf))
		return 0;
	else
		return 1;      
}
...
{
	if(HALIIC_WriteByteToSlave(addr,reg,data))
		return 0;
	else
		return 1;
}
...
{
  u8 res;
	if(HALIIC_ReadByteFromSlave(addr,reg,&res))
		return res;
	else
		return 0;
}
